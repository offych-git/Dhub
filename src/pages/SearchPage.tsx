...
const SearchPage: React.FC = () => {
  ...
  const searchItems = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === "deals") {
        itemsToSet = data?.map((d) => ({ id: d.id, title: decodeHtmlEntities(d.title), currentPrice: parseFloat(d.current_price), originalPrice: d.original_price ? parseFloat(d.original_price) : undefined, store: { id: d.store_id, name: d.store_id }, category: { id: d.category_id, name: d.category_id }, image: d.image_url, postedAt: { relative: formatTimeAgo((d.created_at || '').replace(' ', 'T').replace(/\+00$/, 'Z')), exact: new Date(d.created_at).toLocaleString(), }, popularity: d.vote_count || 0, comments: d.deal_comments?.length || 0, postedBy: { id: d.profiles?.id || "anon", name: d.profiles?.display_name || d.profiles?.email?.split("@")[0] || "Anonymous", avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(d.profiles?.display_name || d.profiles?.email?.split("@")[0] || "A")}&background=random`, }, description: d.description, url: d.deal_url, createdAt: d.created_at, expires_at: d.expires_at, type: "deal", status: d.status, })) || [];
                      ...
                      <div className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="text-gray-400 text-xs" title={ promo.created_at ? new Date(promo.created_at).toLocaleString() : undefined } >
                            {formatTimeAgo((promo.created_at || '').replace(' ', 'T').replace(/\+00$/, 'Z'))}
                          </div>
