...
const AdminActions: React.FC<AdminActionsProps> = ({ type, id, userId, createdAt, onAction, className }) => {
  ...
  const handleMarkAsExpired = async () => {
    ...
    const confirmExpire = window.confirm(`Вы уверены, что хотите отметить этот ${type === 'deal' ? 'товар' : type === 'promo' ? 'промокод' : type === 'sweepstakes' ? 'розыгрыш' : 'элемент'} как истекший?`);
    ...
    if (!id) {
      console.error("Mark as expired failed: ID is required");
    ...
    const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    if (!uuidPattern.test(id)) {
      console.error("Mark as expired failed: Invalid ID format", id);
      setIsMarkingExpired(false);
      return;
    }
    ...
    try {
      ...
      if (type === 'deal') {
        ...
        alert("Этот тип элемента не может быть отмечен как истекший.");
        setIsMarkingExpired(false);
        return;
      }
      ...
      alert(`${type === 'deal' ? 'Скидка' : type === 'promo' ? 'Промокод' : type === 'sweepstakes' ? 'Розыгрыш' : 'Элемент'} успешно отмечен как истекший`);
      ...
      console.error("Error during marking as expired:", error);
      alert(`Unexpected error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsMarkingExpired(false);
    }
  ...
  return (
    <div className={`relative ${className || ''}`} ref={menuRef}>
      ...
      {isMenuOpen && (
        <div className="fixed right-2 top-auto bg-gray-800 border border-gray-700 rounded-md shadow-lg z-[9999] min-w-[140px]" 
             style={{
               top: menuRef.current ? menuRef.current.getBoundingClientRect().bottom + window.scrollY + 4 : 'auto',
               right: menuRef.current ? window.innerWidth - menuRef.current.getBoundingClientRect().right + window.scrollX : '8px'
             }}>
          {canEdit && (
            <button
              onClick={handleEdit}
          ...
          {canMarkAsExpired && (
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-orange-400 hover:bg-gray-700 flex items-center gap-2"
            >
