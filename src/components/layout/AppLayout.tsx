...
import { supabase } from "../../lib/supabase"; // Добавлен импорт supabase
...
const AppLayout: React.FC = () => {
  ...
  const [userDisplayName, setUserDisplayName] = useState<string | null>(null); // Новое состояние для имени
  ...
  useEffect(() => {
      ...
      if (user?.id) {
        try {
          const { data, error } = await supabase
            .from('profiles')
            .select('display_name, email') // Выбираем display_name и email
            .eq('id', user.id)
            .single();
    ...
    fetchDisplayName();
  }, [user?.id, user?.email]); // Запускать эффект при изменении ID или email пользователя
  ...
  return (
    <div className={`min-h-screen relative`}>
      ...
      {/* Начинаем React Fragment для обертывания соседних элементов */}
      <>
        {user && (
          <div
          ...
          >
            <div className="p-4 border-b border-gray-700 flex-shrink-0">
              <div className="flex items-center">
                <div className="w-10 h-10 rounded-full bg-gray-700 overflow-hidden">
                  <img
                    src={`https://ui-avatars.com/api/?name=${encodeURIComponent(userDisplayName || user.email || "User")}&background=random`}
                    alt={userDisplayName || user.email || "User Avatar"}
            ...
            {/* Этот блок закрыт здесь, но он был внутри предыдущего div.
                Предыдущий код имел:
                <div className="text-white font-medium">
                  {user.user_metadata?.display_name || user.email?.split("@")[0]}
                </div>
                <div className="text-gray-400 text-sm">{user.email}</div>
                <div className="text-gray-400 text-sm">{user.email}</div> // лишний div
                Я исправил это, убрав лишний div.
            */}
            <div className="p-4 border-b border-gray-700 flex-shrink-0">
        ...
        )} {/* Закрытие условного рендеринга для isSidebarOpen */}
